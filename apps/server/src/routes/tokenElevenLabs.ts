 ...
 *
 * Uses Scribe v2 Realtime model for ultra-low latency (<150ms) transcription.
...
function buildWebSocketUrl(token: string): string {
  const params = new URLSearchParams({
    token: token,
    // Use Scribe v2 Realtime for live transcription (not scribe_v1 which is batch-only)
