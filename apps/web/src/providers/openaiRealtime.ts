...
export class OpenAIRealtimeAdapter extends BaseProviderAdapter {
   ...
   *
   * Required subprotocols for preview models:
   * - 'realtime' - base protocol
   * - 'openai-insecure-api-key.{token}' - authentication
   * - 'openai-beta.realtime-v1' - required for preview/beta models
   *
  ...
  protected override getWebSocketProtocols(): string[] {
    ...
    // Three subprotocols required for preview models
    return [
      'realtime',
   ...
   *
   * NOTE: When using ephemeral tokens from /v1/realtime/client_secrets (GA API),
   * the session is already configured with transcription settings during token creation.
   * No additional session.update is needed - the session is ready to receive audio.
   */
  protected createConfigurationMessage(): string | null {
    // GA API: Session is already configured when ephemeral token is created
    // The token endpoint sets up transcription model, language, and turn_detection
    // Sending session.update here would cause "missing_required_parameter" errors
    // because the GA API uses different parameter names/structure
    this.log('Session pre-configured via token - no update needed');
    return null;
  }
